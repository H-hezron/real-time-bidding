...
export async function transferICP(advertiserAddress, amount, memo) {
  const canister = window.canister.ledger;
  const account = AccountIdentifier.fromHex(advertiserAddress);
...
export async function balance() {
  const canister = window.canister.ledger;
  const address = await window.canister.advertManager.getAddressFromPrincipal(
